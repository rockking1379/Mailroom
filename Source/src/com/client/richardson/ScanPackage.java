...
public class ScanPackage extends JFrame
{
	...
	public ScanPackage(DatabaseManager manager, String loggedIn)
	{
		...
		/*
		 * comboBox.setModel(new DefaultComboBoxModel(new String[] {"AAO",
		 * "Academic Affairs", "Admissions\t", "AITC", "Alumni/Foundation",
		 * "Art", "AS&F", "Bookstore", "Business Office", "Communications",
		 * "Community Partnership", "Computing Services", "Counseling & Career",
		 * "Counselor Education", "EEO", "English/ Communication", "Enrollment",
		 * "Extended Studies", "Facilities Office", "Facilities Warehouse",
		 ...
		 * itemStateChanged(ItemEvent arg0) { Object tsub =
		 * comboBox.getSelectedItem(); String sub = tsub.toString();
		 * 
		 * if(sub=="SUB Mailroom"){ lblBox.setVisible(true);
		 * BoxText.setVisible(true); }else{ BoxText.setVisible(false);
	...
	public class SaveListener implements ActionListener
	{
		public void actionPerformed(ActionEvent arg0)
		{
				...
				if (pac.getTrackNum().equals(TrackText.getText()))
				{
					JOptionPane
							.showMessageDialog(null,
									"The package number you have scanned in is already associated with a package.");
					return;
				}
			...
			if (selectedPerson == null)
			{
				...
				selectedPerson = new Person(NameText.getText(), LastNameText
						.getText(), "Unknown@", "Unknown", BoxText.getText(),
						stop);
				manager.addPerson(selectedPerson);
				// manager.createPerson(sps);
				p = new Package(NameText.getText(), LastNameText.getText(),
	...
	public class AutoFillListener implements ActionListener
	{
		...
		public void actionPerformed(ActionEvent e)
		{
			ArrayList<Person> p = (ArrayList<Person>) manager.findPerson(
					NameText.getText(), LastNameText.getText());
			if (p == null)
			{
				JOptionPane.showMessageDialog(null,
						"no one was found by that name.");
				return;
			}
			if (p.size() == 0)
			{
